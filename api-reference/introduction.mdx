...
* Ziplime allows users to ingest data into bundles using an API key from Limex DataHub. This enables access to a wide range of financial datasets for backtesting. To obtain the necessary API key, visit 

  [Limex DataHub](https://datahub.limex.com/)
...
* For live trading, you do not need an API key from Limex DataHub as it does not rely on data bundles. However, you must have a Lime account and configure your 
  ...
  `credentials.json`
   ...
   file with the necessary account information.

* Save this file as an environment variable:
...
```
poetry run python -m ziplime run -b limex_us_equity --start 2024-12-01 --end 2024-12-13 --data-frequency daily --capital-base 100000 --no-benchmark --broker lime-trader-sdk -f /Users/live_algo.py --print-algo
```

These features are designed for more advanced users who prefer to build their own customized trading setups. The Ziplime library can be found on PyPI [here](https://pypi.org/project/ziplime/), providing a robust and flexible toolset for building sophisticated trading systems.
