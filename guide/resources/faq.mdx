...
<AccordionGroup>
  ...
  <Accordion title="What about data security?">
    All computation runs in the cloud, and results and data are stored inside the company's own environment. The Russian and US environments are fully independent. The platform uses serverless infrastructure.
  </Accordion>
  ...
  <Accordion title="How is Ziplime different from Finam DMA and Finam Trade API?">
    These serve different people. DMA is for technical teams that need minimal latency (HFT, market making, arbitrage). Trade API is a bare API for people who build everything themselves. Ziplime is a ready-made, AI-powered environment for quantitative strategies, for users who care about the idea and the math rather than execution speed. See [How We're Different](/guide/product/how-we-are-different).
  </Accordion>

  <Accordion title="What markets and instruments are supported?">
    The Russian environment covers stocks, mutual funds, futures, and bonds; the US environment covers ETFs and stocks. History is available from 2005 (no delay in Russia, a 15-minute delay in the US).
  </Accordion>
  ...
  <Accordion title="How do I go live with a strategy?">
    From the interface, in one or two clicks: through Finam Trade API in Russia, or through the broker Lime in the US, with a short setup of trading parameters.
  </Accordion>
</AccordionGroup>
