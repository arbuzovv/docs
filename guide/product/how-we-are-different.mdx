...
description: "Ziplime vs. generic AI assistants and vs. other Finam products"
---
...
## vs. other Finam products (DMA and Trade API)

These three Finam products for active trading serve different people and don't compete with each other.

| Product | Who it's for | Main value | Typical use case |
| --- | --- | --- | --- |
| **Finam DMA** | A narrow group of technical teams (roughly 50 users) | Minimal latency, direct exchange access | High-frequency trading, market making, arbitrage |
| **Finam Trade API** | Users who build everything themselves | Flexibility and fine control, a "bare" API | Custom-built trading systems |
...
If a customer needs minimal latency, high-frequency trading, or market making, point them to **DMA**. If they want to build everything from scratch on a bare API, point them to **Trade API**. Ziplime fits users for whom the idea and the math of the strategy matter more than execution speed or infrastructure.

Ziplime and Trade API audiences can overlap, but they're different types of users: Trade API users like to build everything by hand, while Ziplime users want the AI to own the environment and the code. Note that for live trading in Russia, Ziplime itself uses Finam Trade API under the hood — it just hides all of that complexity behind a simple interface.